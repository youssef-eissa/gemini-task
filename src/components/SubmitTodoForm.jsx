...
async function submitTodo(prevState, formData) {
 ...
 <form action={formAction} className="border border-mainColor p-3 rounded flex flex-col gap-3">
    <input value={todoData.title} onChange={e=>setTodoData({...todoData,title:e.target.value})} autoComplete="off" type="text" name="title" placeholder="Add Todo Title" className={` bg-inputBG p-2 rounded-lg placeholder:text-placeholderColor sm:text-base text-xs text-mainColor outline-none ${state?.title&& "border border-red-500"}`}/>
    {state?.title&&<span className="text-red-500 text-sm">{state.title}</span>}
     <textarea value={todoData.description} onChange={e=>setTodoData({...todoData,description:e.target.value})} autoComplete="off" type="text" name="description" placeholder="Add Todo Description" className={` bg-inputBG p-2 rounded-lg sm:text-base text-xs placeholder:text-placeholderColor text-mainColor resize-none h-[100px] outline-none ${state?.description&& "border border-red-500"}`}/>
